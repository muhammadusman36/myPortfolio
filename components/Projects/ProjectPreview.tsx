...
export default function ProjectPreview({ modal, projects }: ProjectModalProps) {
  ...
  return (
    <>
      ...
      ></motion.div>
      <motion.div
        className="font-base bg-primary pointer-events-none absolute z-10 flex h-16 w-16 items-center justify-center rounded-full bg-transparen font-light text-white"
        ref={cursorLabel}
        variants={scaleAnimation}
        initial="initial"
        animate={active ? 'enter' : 'closed'}
